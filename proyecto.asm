...
.data
    ...
    msgName db "Sergio Giovanni Castro Funes" , "$" ; insertar nombre aca 
    msgReg db "201800723" , "$" ; insertar carnet aca 
	msgUno db "1" , "$"
    texto db 50 dup("$");DE ACA PARA ABAJO SON VARIABLES QUE USO PARA EL MENU
	msg1    db      10, 13, 10, 13, "Menu :",0Dh,0Ah,0Dh,0Ah,09h
			db      "F1- login",0Dh,0Ah,09h
			db      "F5- register",0Dh,0Ah,09h     
			db      "SPACE- salir",0Dh,0Ah,09h    
			db      "presione la tecla deseada : "
			db      '$' 
    msglogin    db      10, 13, 10, 13, "Login :",0Dh,0Ah,0Dh,0Ah,09h
			db      "usuario: "
			db      '$'     
    msgPass        db 0Dh,0Ah,0Dh,0Ah,09h, 'contrase', 164, 'a: ', '$'                 
